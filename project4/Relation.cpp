...
//do I need to add this new relation to database or I guess it just updates the one already in there
Relation Relation::unionize(Relation toUnion) {
    Relation result;
    fillRelation(result);
    set<Tuple> unionSet = toUnion.getSet();
    for(Tuple t: unionSet) {
        
        if (result.my_set.insert(t).second) {//this does the insert and then tells you if it inserted
                                        //second is a boolean saying whether it inserted or not
            cout << t.toString(result.my_header);
        }
        else {
            //do nothing 
        }
